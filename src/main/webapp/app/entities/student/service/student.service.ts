...
type RestOf<T extends IStudent | NewStudent> = Omit<T, 'startDate'> & {
...
export class StudentService {
  ...
  protected convertDateFromClient<T extends IStudent | NewStudent | PartialUpdateStudent>(student: T): RestOf<T> {
    return {
      ...student,
      startDate: student.startDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restStudent: RestStudent): IStudent {
    return {
      ...restStudent,
      startDate: restStudent.startDate ? dayjs(restStudent.startDate) : undefined,
    };
  }
